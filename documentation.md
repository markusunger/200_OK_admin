...
Your API automatically assigns an incrementing `id` field to that data and makes it available both as part of the resource collection (`/users`) and an individual collection item (`/users/1`).

You can issue request for all of the basic CRUD operations:
...
Resources can be nested up to four levels deep, and each subresource is specific to an item of the parent resource.
...
In order to create an API with _200 OK_, you do not need to register or signup. It's totally free and you can get your own API in a matter of seconds by clicking the big, shiny button on the frontpage. If you have no need for any of the helpful tools that _200 OK provides, you only need to grab your API URL (in the form of `https://<your-API-name>.200ok.app/` and you're ready to make requests to it.
...
You might also find yourself a paticipant in a hackathon, and you need a _fire-and-forget_-type of backend without any long-lived persistence. 200 OK is a good fit for that use case as well. 
...
A simple resource can be accessed under the URI `/<resource-name>`. So a resource of users would have a fully qualified URL of

```https://<your-API-name>.200ok.app/users```

This would represent a resource collection, comprising a varying number of individual users.

**Note** - For the purposes of readability, the documentation will use the shorter **URI** format for identifying resources and resource items, e.g. `/users/5/images`. To issue any valid request, you will of course have to provide your tool with the fully qualified **URL** to your API resources in the form of `https://<your-API-name>.200ok.app/users/5/images`.

Each item of a resource collection automatically receives an identifier in the form of an incrementing integer value (`1`, `2` and so on). The identifier is added automatically to each item when it is stored. That identifier can be used in the form of the URI `/<resource-name>/<resource-identifier>` to access an iniividual resource item.
...
In total, you can nest resources up to **4 levels deep**. 
 
### Using HTTP requests 

A _200 OK_ API assumes that each resource for which a request is received should exist. For that reason it is not necessary to explicitly create or delete a resource collection. Instead, any of the following operations will work on any resource, whether it has been explicitly queried before or nor not.

There are four kinds of action available on a resource collection or individual item, represented by an HTTP verb used in performing an request to an API. 
 
- [Retrieve collections or items](#retrieval) (`GET`) 
- [Create collection items](#creation) (`POST`) 
- [Update collection items](#update) (`PUT`) 
- [Delete collection items](#deletion) (`DELETE`)

The data format for a _200 OK_ API is JSON (_JavaScript Object Notation_), so each resource item is represented by a valid JSON object, while resource collections will consist of an array of JSON objects.
...
A complete, valid HTTP POST request to a `users` resource might look like the following:
...
A resource can either be retrieved as a whole (comprising all of its items) or by specifically requesting a certain resource item. This is done by issuing a `GET` request, which is the default request type for most browsers or HTTP communication tools (like the `curl` command line tool or the in-browser `fetch` method).

So a `GET` request to `/users` would return the whole `users` resource collection as an array of JSON objects, while the same request to `/users/1337` would only return a JSON object for the item from `users` with the identifier `id` of `1337`.
...
A successful response to a `GET` request will containthe requested data as well as a _200 OK_ status code.
...
In that case, the request body must contain valid JSON data which will be stored as is, with one notable exception: _that _200 OK_ automatically issues an `id` field for any newly created item. This operation will overwrite any `id` field values that you provide. This acts a guarantee for consistent data and will prevent any accidental duplication of `id`s that might render parts of your resource collection inaccessible.

The response to a `POST` request will contain the newly created item (consisting of the data you sent as well as the newly created `id` field). This response will carry a _201 Created_ status code.
...
The response to that request would contain the following JSON body: 
...
The resource item would afterwards look like this:
...
While the automatic RESTful operation mode will be sufficient for a big set of users, the need for more fine-grained control over your API might arise. Thus _200 OK_ provides a set of helping tools that enable better debugging and configuration of an API.
...
You can have a **maximum number of seven APIs** connected to your account. If you have reached the maximum amount, you need to wait until one of your APIs expires. This happens **seven days after its creation**, at which point it will automatically be removed from your account.

You can select a specific API by clicking on it in the _Your APIs_ list. On the right side, you will now see details about that API (like the creation date and how much time is left before it expires). You can also select the _Inspect Requests/Responses_ tool as well as the _Customize Endpoint Behavior_ feature.

### Inspect Requests and Responses

When first starting the inspection tool, there will not be much to look at. But every request that you issue to your API from that point in time until you close the window, will be logged live on the page.
...
Those are not resources (there should be no collection `login` with items like `/login/1`), so the standard behavior won't match the intended use.
...
The _endpoint path_ field defines the URI of your custom endpoint and you can immediately seethe full URL under which that endpoint will be available later. For the login example above, we would therefore enter `/login` into that field.
...
If you enable a certain HTTP method, you can define a JSON response with which your API is going to respond. Please note that you are required to enter [valid JSON data](https://de.wikipedia.org/wiki/JavaScript_Object_Notation) as will be determined by trying to JSON-parse your input. If your input is malformed or contains characters or data types not supported by JSON, you will receive a warning message and will be unable to save your custom endpoint.

Once youare satisfied with your custom endpoint and have passed validation for your JSON response data, you can click _Save Changes_ and will be able to make requests to that endpoint immediately to receive the custom responses.
